...
get_uniform <- function(outcomes, metadata, trt_unit=1) {
    ...
    data_out <- format_data(outcomes, metadata, trt_unit)$synth_data

    ## impute "synthetic control" with uniform weights
    controls <- data_out$Y0plot
    n_c <- dim(controls)[2]
    weights <- rep(1/n_c, times=n_c)

    ## compute square error objective function
    primal_obj <- sqrt(sum((data_out$Z0 %*% weights - data_out$Z1)^2))
    ## primal objective value scaled by least squares difference for mean
    x <- t(data_out$Z0)
    y <- data_out$Z1
    ...
    ## finalize output
    ctrls$primal_obj <- primal_obj
    ctrls$scaled_primal_obj <- scaled_primal_obj
    return(ctrls)
}
