...
fit_ridgeaug_cov_formatted <- function(ipw_format, syn_format,Z,
    ...
    Z_1 <- colMeans(Z_cent[trt==1,,drop=FALSE])
    ## use CV to choose lambda if it's null
    if(is.null(lambda)) {
        if(ncol(y) > 1) {
            lambda <- glmnet::cv.glmnet(Z_c, cbind(X[trt==0,,drop=FALSE], y[trt==0,,drop=FALSE]),
                                        alpha=0, family="mgaussian")$lambda.min
            fit <- glmnet::glmnet(Z_c, cbind(X[trt==0,,drop=FALSE], y[trt==0,,drop=FALSE]),
                                        alpha=0, lambda=lambda, family="mgaussian")
        } else {
            lambda <- glmnet::cv.glmnet(Z_c, cbind(X[trt==0,,drop=FALSE], y[trt==0]),
                                        alpha=0, family="mgaussian")$lambda.min
            fit <- glmnet::glmnet(Z_c, cbind(X[trt==0,,drop=FALSE], y[trt==0]),
                                  alpha=0, lambda=lambda, family="mgaussian")
        }
    }
    ## regress out covariates
    Xc_hat <- predict(fit, Z_c)[,1:ncol(X),1]
    X1_hat <- colMeans(predict(fit, Z[trt==1,,drop=F])[,1:ncol(X),1,drop=F])
