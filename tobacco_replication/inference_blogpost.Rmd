...
There are many competing approaches to inference in the Synthetic Control world, and these apporaches rely on a variety of different sets of assumptions that researchers can get quite agitated about.
...
With synthetic control and treatment impacts in hand, the second step is to figure out how to conduct inference: we have deliberately manufactured a "synthetic" unit to look like our treated unit, and now we want to know if the difference in their behaviors post-treatment could be due to random chance, or something structural like a treatment effect.


## A note on different packages and approaches

In terms of R packages for synthetic control, we know of two contenders.
...
Before diving into inference, we first discuss how Synth and augsynth are different.

To use the `Synth` package, you specify a set of features you want to match on, and a separate set of pre-treatment "test" outcomes to assess performance on, and then the package will find the best weighting of the match features that produces the closest matched synthetic control as measured by the specified pre-treatment "test" outcomes.
This results in two sets of weights, the "V" weights, which weight the possible match variables, and the match weights, which weight the donor units.
This approach makes it easy to include both lagged outcomes *and* auxiliary covariates, as the V weights will essentially rescale covariates to optimize tradeoffs between which covariates to prioritize when building a match.

The `augsynth` package, by contrast, directly finds unit weights to minimize the mean squared difference of all pre-treatment outcome periods and any additional covariates.
The scaling of match covariates can be done as a pre-processing step, but is not done within the package.
Instead of focusing on V weights, the `augsynth` package allows for balance correction via a (usually ridge) regression step.
This can help regularize weights so, in the case of rich donor unit contexts, more donor units can be included in the synthetic match.
...
```{r reformat tobacco data using tidyverse, include = FALSE}
tobacco <- Wk.data %>%
...
The default `augsynth()` call conducts bias removal via a ridge regression.
You can prevent it, which is closer in spirit to the Abadie approach, by including `progfunc = "none"`.
However, in this case, dropping the bias removal gives results that deviate more strongly from the tobacco paper, but if we allow augsynth to do its default bias removal, we more closely replicate the paper.
We hypothesize that the bias removal is serving the role of adjusting for the measured covariates and handling the variable importance weights (the bias removal being a regression that will naturally weight some variables more than others).
Without that step, all the pre-treatment outcomes and weights are included equally.



### Getting our inference

We get our inference via summarizing.
Inference options include those already included in the package (`conformal`, `jackknife`, `jackknife+`), but also `permutation` and `permutation_rstat` (RMSPE-adjusted permutation inference), which are calculated using placebo tests. 
