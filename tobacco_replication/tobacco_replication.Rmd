...
This project currently lives on the [n-delaney/augsynth_rstat](https://github.com/n-delaney/augsynth_rstat) GitHub repository. All changes should (ideally) be backwards compatible, although they are only currently implemented for cases where `single_augsynth()` is executed. The proposed extensions to the `augsynth` package do the following:

* Add inference output by default to `augsynth` objects, stored in  `augsynth$results`.
* Extend the inference options available for `augsynth` objects with a single treated unit. Additional options now include `permutation` and `permutation_rstat` (RMSPE-adjusted placebo tests). 
* Modify  `summary.augsynth()` 
  * If an `augsynth` does not already have inference results, `summary.augsynth()` will calculate them. 
  * It's also possible to use `summary.augsynth()` to re-do inference on an existing model with different characteristics (for example, switching from the default of `conformal` to `permutation`).
...
* Add flexibility to `plot.augsynth()` 
  * Allow users to choose from inference types 
  * Include flags to include confidence intervals, the ATT only, or ATT and placebo tests.
...
The proposed extensions to plotting capabilities include the possibility to customize the inferential features of plots and to easily generate plots showing placebo tests, when appropriate.

Like the updates to `summary.augsynth()`, changes to `plot.augsynth()` use the inferential features of the provided `augsynth` object, or otherwise allow users to specify the desired inferential features
...
Setting `ci = FALSE` with inference types of `permutation` or `permutation_rstat` will return a plot showing each placebo test. Otherwise, for other inference types, it will return only the ATT estimate. 

```{r fig.width=4.5, fig.height=4}
plot(syn, 
     inf_type = 'permutation',
     ci = F # option to turn off 95% confidence intervals
     ) + ggtitle("Replication of Abadie et al. (2010), Figure 4")
```


```{r  fig.width=4.5, fig.height=3.25}
p <- plot(syn, ci = F) + ggtitle("Replication of Abadie et al. (2010), Figure 3")
```
...
If a gap plot is desired, the user can call `augsynth_gap_plot()` on an `ausynth` object.

```{r  fig.width=4.5, fig.height=4}
p <- augsynth_gap_plot(syn_rstat, measure = 'synth') + 
...
By default, `augsynth_gap_plot()` will return both the raw average of donor units and the synthetic counterfactual.

```{r Gap plot with average, echo=TRUE, fig.show="hold", fig.align='default', fig.height=4, fig.width=4.6}
augsynth_gap_plot(syn_rstat)
```
...
update_augsynth(syn) %>% # drops units with >20x treated RMSPE by default
  plot(ci = F, inf_type = 'permutation') + 
  ...
  annotate('text', y = -48, x = 1970, label = "Removes donors with 20x California's  pre-treatment RMSPE",
           hjust = 0, color = 'darkseagreen', size = 3) 
update_augsynth(syn, drop = 5) %>% plot(ci = F, inf_type = 'permutation') + 
  ...
  annotate('text', y = -48, x = 1970, label = "Removes donors with 5x California's pre-treatment RMSPE",
         hjust = 0, color = 'darkseagreen', size = 3) 
update_augsynth(syn, drop = 2) %>% plot(ci = F, inf_type = 'permutation') + 
...
update_augsynth(syn, drop = drop_states) %>% plot(ci = F, inf_type = 'permutation') 
```
